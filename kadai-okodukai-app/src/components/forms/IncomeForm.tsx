...
import type { IncomeInput } from "../../App";

type Props = {
  onSubmit: (payload: IncomeInput) => Promise<void>;    // 親に「保存してね」とお願いするための関数
};
...
export default function IncomeForm({ onSubmit }: Props) {
  ...
  useEffect(() => {
  // 復元
    const saved = localStorage.getItem("incomeForm");
    if (saved) {
  ...
  }, [amount, selectedCategory, date, memo]);
  ...
  const handleAdd = async () => {
    ...
    setSelectedCategory("");
    setMemo("");
    localStorage.removeItem("incomeForm");
  };
  ...
  return (
    <div className="p-[30px_20px] border border-[var(--color-meter-border)] shadow-[var(--color-shadow)] rounded-[10px] space-y-10 bg-white">
      ...
      {/* カテゴリー */}
      <div>
        <label className="flex items-center gap-5 mb-5">
          カテゴリー <RequiredLabel />
        </label>
